...
export default {
    ...
    methods:{
        toggleFavorite(){
            ...
            localStorage.setItem("seller-favorite",JSON.stringify(this.favorite))  
            //刷新后仍可以获得数据，缓存在本地
        }

    },
    created(){
        this.favorite = JSON.parse(localStorage.getItem("seller-favorite"))
    },
    components:{
        Split,
        Star,
    ...
    computed:{
        seller() {
                return this.data.seller || {} //props传过来的  data是一个对象{seller：{数据}}，故现在computed进行计算一下
        },
        favoriteText(){
